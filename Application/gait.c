...
//    float half_S = gait->stride_length ;
    ...
    float start_total_cycles = leg->phase_offset;
    float next_boundary = (start_total_cycles < 0.5f) ? 0.5f : 1.0f;
    uint8_t is_first_phase = (leg_total_cycles < next_boundary) ? 1 : 0;

    if (t_norm < 0.5f) {
        phase_is_swing = 1;         // 抬腿摆动
        dt = t_norm * 2.0f;
        // 只有真正意义上的第一步，才从原点起步，否则平滑衔接
        current_x_start = is_first_phase ? leg->x_base : swing_x_start;
        current_x_end = swing_x_end;
    } else {
        phase_is_swing = 0;         // 落地支撑
        dt = (t_norm - 0.5f) * 2.0f;
        current_x_start = is_first_phase ? leg->x_base : swing_x_end;
        current_x_end = swing_x_start;
    }
